...
Hooks:PostHook(CoreBodyDamage, "init", "sh_init", function (self)
	if self._body_element and getmetatable(self._unit:character_damage()) == TankCopDamage then
		if not CoreBodyDamage._tank_armor_multiplier then
			...
			CoreBodyDamage._tank_glass_multiplier = 1 / math.max(1, hp_muls[diff_i] * 0.5)
		end
